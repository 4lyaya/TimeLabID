...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-primary">Daftar Jadwal Lab</h1>
        <a href="{{ route('petugas.schedules.create') }}"
            ...
            @foreach ($schedules as $schedule)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 flex gap-2">
                        <a href="{{ route('petugas.schedules.edit', $schedule) }}"
                            class="text-blue-600 hover:underline">Edit</a>
                        <form action="{{ route('petugas.schedules.destroy', $schedule) }}" method="POST"
